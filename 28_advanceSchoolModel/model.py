...
## ACADEMIC RELATED MODEL ENDS HERE! 
## LIBRARY RELATED MODEL STARTS HERE!
# Library Book model
...
## LIBRARY RELATED MODEL ENDS HERE!
...
## CAFETERIA MODEL ENDS HERE !
## HOSTEL MODEL STARTS HERE !
...
## HOSTEL MODEL ENDS HERE !

## TRANSPORT MODEL STARTS HERE!    
...
class TransportRoute(models.Model):
    ...
    stops = models.TextField(help_text="Enter stops separated by commas")
...
class MedicalIncident(models.Model):
    ...
    medic = models.ForeignKey(Medic, on_delete=models.CASCADE)  # Medic responsible
...
class FirstAidSupply(models.Model):
    ...
    medic = models.ForeignKey(Medic, on_delete=models.CASCADE, related_name='supplies')

    def __str__(self):
        return f"{self.name} - {self.quantity} available"

class MedicalHistory(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)  # Could be student or staff
...
## MEDICS RELATED MODEL ENDS HERE!
...
class StudentPayment(models.Model):
    student = models.ForeignKey(StudentProfile, on_delete=models.CASCADE)
    parent = models.ForeignKey(ParentProfile, on_delete=models.CASCADE)  # Parent making the payment
...
class PaymentInvoice(models.Model):
    ...
    issued_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, limit_choices_to={'user_type': 7})  # Accountant

def __str__(self):
    return f"Invoice for {self.student.user.get_full_name()} - {self.total_due} ({self.status})"
