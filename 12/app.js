...
function updateFruitList() {
  ...
  // Use map to transform the fruits array into formatted list items
  const fruitItems = fruits.map((fruit) => {
    const formattedFruit = formatFruit(fruit, format);
    return `<li>${formattedFruit}</li>`;
  });

  // Join the items and set the innerHTML of the list
  fruitList.innerHTML = fruitItems.join("");
}
